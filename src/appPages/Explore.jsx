...
const NAV_PAGES = [
  { label: "Dasboard",       icon: "⌂",  desc: "Back to Dasboard" , to : "/"     },
  { label: "Editor",     icon: "⟨/⟩",desc: "Write & run code"   , to:"/editor"       },
  { label: "Explore",    icon: "✦",  desc: "Browse the community", to:"/explorer",  active: true },
];
...
export default function DevioExplorer({ onNavigate }) {
  ...
  const performSearch = useCallback(async (searchQuery, sortBy, tag) => {
  ...
  const debouncedSearch = useRef(
    debounce((searchQuery) => {
      performSearch(searchQuery, sortRef.current, tagRef.current);
    }, 500)
  ).current;
  ...
  const handleSortChange = (newSort) => {
    ...
    setTimeout(() => {
      performSearch(queryRef.current, newSort, tagRef.current);
    }, 0);
  };
  ...
  const handleTagChange = (newTag) => {
    ...
    // Use setTimeout to ensure state is updated before search
    setTimeout(() => {
      performSearch(queryRef.current, sortRef.current, newTag);
    }, 0);
  };
  ...
  const handleClearTag = () => {
    setActiveTag("");
    setTimeout(() => {
      performSearch(queryRef.current, sortRef.current, "");
    }, 0);
  };
  ...
  const handleClearSort = () => {
    setSort("recents");
    setTimeout(() => {
      performSearch(queryRef.current, "recents", tagRef.current);
    }, 0);
  };
    ...
    <div style={{
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        <div style={{
          ...
          background: `
            radial-gradient(ellipse 55% 40% at 10% 0%,  rgba(29,78,216,0.09) 0%, transparent 55%),
            radial-gradient(ellipse 45% 35% at 90% 100%, rgba(6,182,212,0.05) 0%, transparent 55%)
          `,
        }} />
      ...
      <header style={{
        ...
        <button
          ...
          style={{
            ...
            color: activeFilters > 0 ? "#93c5fd" : "#64748b",
        ...
        {/* Result count */}
        <span style={{ fontSize: 11, color: "#1e3a6e", flexShrink: 0, marginLeft: "auto" }}>
      ...
      <div className="exp-layout" style={{ position: "relative", zIndex: 10 }}>
        ...
        <aside className="exp-sidebar">
          <div>
            <p style={{ fontSize: 10, fontWeight: 700, color: "#334155", letterSpacing: "0.09em", textTransform: "uppercase", marginBottom: 8 }}>Sort by</p>
          ...
          <div>
            <p style={{ fontSize: 10, fontWeight: 700, color: "#334155", letterSpacing: "0.09em", textTransform: "uppercase", marginBottom: 8 }}>Topics</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {TAGS_ALL.map(tag => (
                <button key={tag} onClick={() => handleTagChange(tag)} className="sidebar-btn" style={{
                  ...
                  color: activeTag === tag ? "#93c5fd" : "#334155",
                  fontFamily: "inherit", cursor: "pointer", textAlign: "left",
        ...
        <div>
          <div style={{ marginBottom: 18 }}>
            ...
            <p style={{ fontSize: 12, color: "#334155" }}>Browse what the community is building — like, comment, share.</p>
          </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
              {activeTag !== "" && activeTag !== "All" && (
                <button onClick={handleClearTag} className="filter-pill" style={{
                  ...
                  color: "#93c5fd", fontFamily: "inherit", cursor: "pointer",
                }}>#{activeTag} ×</button>
              )}
              {sort !== "recents" && (
          ...
          {(page > 1 || projects.length === 10) && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginTop: 40 }}>
              ...
              {/* Previous */}
              <button
                onClick={() => getAllPosts(pageRef.current - 1)}
                disabled={page === 1}
                style={{
      ...
      <FilterDrawer
        setSearch={() => performSearch(queryRef.current, sortRef.current, tagRef.current)}
        open={filterOpen} 
        onClose={() => {
          setFilterOpen(false)
        }
